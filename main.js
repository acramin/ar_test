...
function randomizeCanPosition() {
  ...
  // Position relative to camera but ensure it's always in front and at good height 0, 1.6, 0
  canPosition.x = 0;
  canPosition.y = 1.6; // Camera is now at 1.6, so this puts can at 1.6-2.6
  canPosition.z = 0; // Always in front (negative Z from camera)
...
function animate() {
  ...
  if (camera) {
    ...
    camera.rotation.set(adjustedBeta, alpha, 0, "YXZ"); // Removed to prevent camera rotation with device orientation
  }
  if (can && !foundCan && gameStarted && !gameCompleted) {
    // NO ANIMATION - can stays perfectly still at its original position
    can.position.set(canPosition.x, canPosition.y, canPosition.z);
